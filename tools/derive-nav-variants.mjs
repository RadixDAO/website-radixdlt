// Derive the canonical nav block per variant from the BUILT site (REBUILD-PLAN.md
// Task 2.2). Reusable: rerun any time dist/ changes to re-check that every
// nav-bearing page still reduces to one of the known variants.
//
//   node tools/derive-nav-variants.mjs [--write]
//
// Without --write: reports the variant groups found in dist/ and FAILS if any page's
// nav doesn't match a known variant (tools/lib/nav-variants.mjs) or if two pages
// claiming the same variant aren't byte-identical after stripping current-marking.
// With --write: also (re)writes src/chrome/nav.<name>.html from the first page seen
// in each group.
//
...
console.log(`${files.length} pages built; ${files.length - noNav} nav-bearing, ${noNav} without a nav\n`);
...
for (const [hash, g] of sorted) {
  ...
  if (!name) {
    console.error(`FAIL: unnamed variant ${hash} (n=${g.pages.length}, ${g.canon.length} bytes, e.g. ${g.pages[0]}). Add it to tools/lib/nav-variants.mjs.`);
    ok = false;
    continue;
  }
...
if (WRITE) {
  ...
  for (const [hash, g] of sorted) {
    const name = VARIANT_NAMES[hash];
    if (!name) continue;
    writeFileSync(join(CHROME_DIR, `nav.${name}.html`), g.canon);
  }
...
console.log('\nOK -- every nav-bearing page reduces to a known, named variant.');
